...
class FinancasApp {
  ...
  configurarEventos() {
    ...
    if (botaoSeletor && painelSeletor) {
      ...
      document.addEventListener("click", (e) => {
        ...
        const clicandoNoPainel =
          painelSeletor &&
          painelSeletor.style.display !== "none" &&
          painelSeletor.contains(e.target);
        ...
        if (painelSeletor && painelSeletor.style.display !== "none") {
          if (clicandoNoPainel || clicandoNoBotao || clicandoNoContainer)
            return;
          painelSeletor.style.display = "none";
  ...
  async obterDadosFinanceiros() {
    try {
      ...
      const responseTransacoes = await fetch(
        "funcoes/transacoes.php?api=transacoes&acao=listar",
      );
      console.log(
        "Response transações:",
        responseTransacoes.status,
        responseTransacoes.statusText,
      );

      if (!responseTransacoes.ok) {
        throw new Error(
          `Erro ao buscar transações: ${responseTransacoes.status}`,
        );
      }

      const textTransacoes = await responseTransacoes.text();
      console.log(
        "Texto da resposta transações:",
        textTransacoes.substring(0, 200),
      );
      ...
      const responseCategorias = await fetch(
        "funcoes/transacoes.php?api=categorias&acao=listar",
      );
      console.log(
        "Response categorias:",
        responseCategorias.status,
        responseCategorias.statusText,
      );

      if (!responseCategorias.ok) {
        throw new Error(
          `Erro ao buscar categorias: ${responseCategorias.status}`,
        );
      }

      const textCategorias = await responseCategorias.text();
      console.log(
        "Texto da resposta categorias:",
        textCategorias.substring(0, 200),
      );
      ...
      const transacoesFiltradas = Array.isArray(transacoes)
        ? transacoes.filter((t) => {
            ...
            return (
              (!intervalo.inicio || d >= intervalo.inicio) &&
              (!intervalo.fim || d <= intervalo.fim)
            );
          })
        : [];
      let receitas = 0;
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        labelsMensal.push(
          `${nomesMeses[d.getMonth()]}/${String(d.getFullYear()).slice(-2)}`,
        );
      }
      ...
      transacoesFiltradas.forEach((transacao) => {
        const ehTransferencia =
          ...
          (typeof transacao.observacoes === "string" &&
            transacao.observacoes.indexOf("TRANSFERENCIA:") === 0);
        ...
        if (!ehTransferencia && transacao.tipo === "receita") {
          if (!categoriasReceitasTotais[categoriaNome]) {
            categoriasReceitasTotais[categoriaNome] = {
              nome: categoriaNome,
              valor: 0,
              cor: categoriaCor,
            };
          ...
          if (!categoriasDespesasTotais[categoriaNome]) {
            categoriasDespesasTotais[categoriaNome] = {
              nome: categoriaNome,
              valor: 0,
              cor: categoriaCor,
            };
  ...
  async criarGraficoLinhas(dados = null) {
    ...
    const searchPeriod = this.obterIntervaloSelecionado();

    if (!dados) {
      dados = await this.obterDadosFinanceiros();
    }

    // DESTROI GRAFICO SE JA EXISTE - REBUILD
    ...
    const inicio = new Date(
      searchPeriod.inicio.getFullYear(),
      searchPeriod.inicio.getMonth(),
      searchPeriod.inicio.getDate(),
    );
    const fim = new Date(
      searchPeriod.fim.getFullYear(),
      searchPeriod.fim.getMonth(),
      searchPeriod.fim.getDate(),
    );

    const dias = [];
    const mapDiaIndex = new Map();
    ...
    const receitasData = new Array(dias.length).fill(0);
    const despesasData = new Array(dias.length).fill(0);
    ...
    if (Array.isArray(dados.transacoes)) {
      dados.transacoes.forEach((t) => {
        const ehTransferencia =
          ...
          (typeof t.observacoes === "string" &&
            t.observacoes.startsWith("TRANSFERENCIA:"));
        ...
        const dt = new Date(dtRaw);
        const dtMidnight = new Date(
          dt.getFullYear(),
          dt.getMonth(),
          dt.getDate(),
        );

        const key = dtMidnight.toISOString().split("T")[0];
        if (!mapDiaIndex.has(key)) return;

        const idx = mapDiaIndex.get(key);
    ...
    // CONSTRUÇÃO DO LAYOUT DO GRAFICO
  ...
  async atualizarListaCategorias(dados = null) {
    ...
    const renderizar = (containerId, categoriasArr) => {
      ...
      if (!categoriasArr || categoriasArr.length === 0) {
        container.innerHTML =
          '<p class="sem-dados">Nenhuma categoria encontrada</p>';
        return;
      }
      const total = categoriasArr.reduce((sum, c) => sum + c.valor, 0);
      categoriasArr.forEach((categoria) => {
        const percentual =
          total > 0 ? ((categoria.valor / total) * 100).toFixed(1) : 0;
